*DECK TEST41
      PROGRAM TEST41
C***BEGIN PROLOGUE  TEST41
C***PURPOSE  Driver for testing SLATEC subprograms
C***LIBRARY   SLATEC
C***CATEGORY  H2
C***TYPE      SINGLE PRECISION (TEST41-S, TEST42-D)
...
C***END PROLOGUE  TEST41
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST41
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST41 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST41 *************')
      END
