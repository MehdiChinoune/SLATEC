*DECK TEST39
      PROGRAM TEST39
C***BEGIN PROLOGUE  TEST39
C***PURPOSE  Driver for testing SLATEC subprograms
C***LIBRARY   SLATEC
C***CATEGORY  H2
C***TYPE      SINGLE PRECISION (TEST39-S, TEST40-D)
...
C***END PROLOGUE  TEST39
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST39
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST39 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST39 *************')
      END
