*DECK TEST45
      PROGRAM TEST45
C***BEGIN PROLOGUE  TEST45
C***PURPOSE  Driver for testing SLATEC subprograms
C            SDRIV1  SDRIV2  SDRIV3
C***LIBRARY   SLATEC
C***CATEGORY  I1A2, I1A1B
C***TYPE      SINGLE PRECISION (TEST45-S, TEST46-D, TEST47-C)
...
C***ROUTINES CALLED  I1MACH, SDQCK, XERMAX, XSETF, XSETUN
C***REVISION HISTORY  (YYMMDD)
C   920801  DATE WRITTEN
C***END PROLOGUE  TEST45
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST45
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
C
C     Test single precision SDRIVE
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST45 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST45 *************')
      END
