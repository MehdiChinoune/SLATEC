*DECK TEST07
      PROGRAM TEST07
C***BEGIN PROLOGUE  TEST07
C***PURPOSE  Driver for testing SLATEC subprograms
C***LIBRARY   SLATEC
C***CATEGORY  C
C***TYPE      SINGLE PRECISION (TEST07-S, TEST08-D)
...
C***END PROLOGUE  TEST07
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST07
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST07 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST07  *************')
      END
