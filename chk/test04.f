*DECK TEST04
      PROGRAM TEST04
C***BEGIN PROLOGUE  TEST04
C***PURPOSE  Driver for testing SLATEC subprograms
C***LIBRARY   SLATEC
C***CATEGORY  C
C***TYPE      COMPLEX (TEST02-S, TEST03-D, TEST04-C)
...
C***END PROLOGUE  TEST04
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST04
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST04 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST04  *************')
      END
