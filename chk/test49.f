*DECK TEST49
      PROGRAM TEST49
C***BEGIN PROLOGUE  TEST49
C***PURPOSE  Driver for testing SLATEC subprograms
C***LIBRARY   SLATEC
C***CATEGORY  I1A2
C***TYPE      DOUBLE PRECISION (TEST48-S, TEST49-D)
...
C***ROUTINES CALLED  DDASQC, I1MACH, XERMAX, XSETF, XSETUN
C***REVISION HISTORY  (YYMMDD)
C   891013  DATE WRITTEN
C   901001  Converted prologue to 4.0 format.  (FNF)
C   901009  Corrected GAMS classification code.  (FNF)
C   901009  Constructed double precision version.  (FNF)
C***END PROLOGUE  TEST49
      INTEGER IPASS, KPRINT, LIN, LUN, NFAIL
C***FIRST EXECUTABLE STATEMENT  TEST49
      LUN = I1MACH(2)
      LIN = I1MACH(1)
      NFAIL = 0
C
...
C
      ...
      ENDIF
      STOP
 9000 FORMAT (/' --------------TEST49 PASSED ALL TESTS----------------')
 9010 FORMAT (/' ************* WARNING -- ', I5,
     1        ' TEST(S) FAILED IN PROGRAM TEST49 *************')
      END
