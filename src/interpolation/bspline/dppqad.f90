...
SUBROUTINE DPPQAD(Ldc,C,Xi,Lxi,K,X1,X2,Pquad)
  ...
  IF ( K<1 ) THEN
    !
    !
    CALL XERMSG('DPPQAD','K DOES NOT SATISFY K.GE.1',2,1)
    RETURN
  ELSEIF ( Lxi<1 ) THEN
    CALL XERMSG('DPPQAD','LXI DOES NOT SATISFY LXI.GE.1',2,1)
    RETURN
  ELSEIF ( Ldc<K ) THEN
    CALL XERMSG('DPPQAD','LDC DOES NOT SATISFY LDC.GE.K',2,1)
    RETURN
  END IF
  aa = MIN(X1,X2)
  bb = MAX(X1,X2)
