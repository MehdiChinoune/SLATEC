...
SUBROUTINE DBINT4(X,Y,Ndata,Ibcl,Ibcr,Fbcl,Fbcr,Kntopt,T,Bcoef,N,K,W)
  ...
  REAL(8) :: Bcoef(Ndata+2), Fbcl, Fbcr, T(Ndata+6), W(5,Ndata+2), X(Ndata), Y(Ndata)
  INTEGER :: i, iflag, ilb, ileft, it, iub, iw, iwp, j, jw, ndm, np, nwrow
  REAL(8) :: tol, txn, tx1, vnikx(4,4), wdtol, work(15), xl
  ...
  ELSE
    ...
    ELSEIF ( Kntopt<1.OR.Kntopt>3 ) THEN
      CALL XERMSG('SLATEC','DBINT4','KNTOPT IS NOT 1, 2, OR 3',2,1)
      RETURN
    ELSE
      K = 4
      N = Ndata + 2
      ...
      IF ( iflag==2 ) THEN
        !
        !
        CALL XERMSG('SLATEC','DBINT4','THE SYSTEM OF EQUATIONS IS SINGULAR',2,1)
        RETURN
      ELSE
      ...
      END IF
    END IF
    50 CALL XERMSG('SLATEC','DBINT4','X VALUES ARE NOT DISTINCT OR NOT ORDERED',2,1)
    RETURN
  END IF
  100  CALL XERMSG('SLATEC','DBINT4',&
