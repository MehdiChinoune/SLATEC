...
SUBROUTINE DBSPPP(T,A,N,K,Ldc,C,Xi,Lxi,Work)
  ...
  !
  INTEGER :: K, Ldc, Lxi, N
  REAL(DP) :: A(N), C(Ldc,N-K+1), T(N+K), Work(K*(N+3)), Xi(N-K+2)
  ...
  IF ( K<1 ) THEN
    CALL XERMSG('DBSPPP','K DOES NOT SATISFY K.GE.1',2,1)
    RETURN
  ELSEIF ( N<K ) THEN
    CALL XERMSG('DBSPPP','N DOES NOT SATISFY N.GE.K',2,1)
    RETURN
  ELSEIF ( Ldc<K ) THEN
    CALL XERMSG('DBSPPP','LDC DOES NOT SATISFY LDC.GE.K',2,1)
    RETURN
  END IF
