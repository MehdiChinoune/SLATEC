...
SUBROUTINE BSPEV(T,Ad,N,K,Nderiv,X,Inev,Svalue,Work)
  ...
  IF ( K<1 ) THEN
    !
    !
    CALL XERMSG('BSPEV','K DOES NOT SATISFY K.GE.1',2,1)
    RETURN
  ELSEIF ( N<K ) THEN
    CALL XERMSG('BSPEV','N DOES NOT SATISFY N.GE.K',2,1)
    RETURN
  ELSEIF ( Nderiv<1.OR.Nderiv>K ) THEN
    CALL XERMSG('BSPEV','NDERIV DOES NOT SATISFY 1.LE.NDERIV.LE.K',2,1)
    RETURN
  ELSE
    id = Nderiv
    ...
    END IF
  END IF
  100  CALL XERMSG('BSPEV','X IS NOT IN T(K).LE.X.LE.T(N+1)',2,1)
  RETURN
