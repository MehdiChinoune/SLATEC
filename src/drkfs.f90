...
SUBROUTINE DRKFS(DF,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,H,Tolfac,Yp,F1,F2,F3,&
  ...
  IF ( Info(1)/=0.AND.Info(1)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DRKFS','IN DDERKF, INFO(1) MUST BE SET TO 0 '//&
  ...
  IF ( Info(2)/=0.AND.Info(2)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DRKFS','IN DDERKF, INFO(2) MUST BE 0 OR 1 '//&
  ...
  IF ( Info(3)/=0.AND.Info(3)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DRKFS','IN DDERKF, INFO(3) MUST BE 0 OR 1 '//&
  ...
  IF ( Neq<1 ) THEN
    WRITE (xern1,'(I8)') Neq
    CALL XERMSG('SLATEC','DRKFS','IN DDERKF, THE NUMBER OF EQUATIONS '//&
  ...
  DO k = 1 , Neq
    IF ( nrtolp==0.AND.Rtol(k)<0.D0 ) THEN
      ...
      CALL XERMSG('SLATEC','DRKFS','IN DDERKF, THE RELATIVE ERROR '//&
    ...
    IF ( natolp==0.AND.Atol(k)<0.D0 ) THEN
      ...
      CALL XERMSG('SLATEC','DRKFS','IN DDERKF, THE ABSOLUTE ERROR '//&
  ...
  IF ( Init/=0 ) THEN
    IF ( T==Tout ) THEN
      ...
      CALL XERMSG('SLATEC','DRKFS','IN DDERKF, YOU HAVE CALLED THE '//&
    ...
    IF ( T/=Told ) THEN
      ...
      CALL XERMSG('SLATEC','DRKFS','IN DDERKF, YOU HAVE CHANGED THE '//&
  ...
  IF ( Idid==(-33) ) THEN
    IF ( Iquit/=(-33) ) THEN
      ...
      GOTO 200
    ELSE
      CALL XERMSG('SLATEC','DRKFS','IN DDERKF, INVALID INPUT WAS '//&
              ...
              IF ( et>0.0D0 ) THEN
                ...
                IF ( .NOT.(Stiff.OR.Nonstf) ) THEN
                  es = ABS&
                    ...
                    (k)+0.023107D0*F3(k)-0.009515D0*F4(k)&
                    +0.003017D0*F5(k)))
