...
!
     ...
     &                'HAVE CALLED THE CODE WITH  RTOL('//xern1//') = '//xern3//
...
!
     ...
     &                'HAVE CALLED THE CODE WITH  ATOL('//xern1//') = '//xern3//
     &                '.  IN THE CASE OF VECTOR ERROR TOLERANCES, '//
...
!
      IF ( Info(4)==1 ) THEN
        IF ( SIGN(1.D0,Tout-T)/=SIGN(1.D0,Tstop-T).OR.ABS(Tout-T)>ABS(Tstop-T) )
     &       THEN
...
!
        IF ( T/=Told ) THEN
          ...
          CALL XERMSG('SLATEC','DDES','IN DDEABM, YOU HAVE '//
     &                'CHANGED THE VALUE OF T FROM '//xern3//' TO '//xern4//
