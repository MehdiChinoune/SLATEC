...
SUBROUTINE DES(F,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,Ypout,Yp,Yy,Wt,P,Phi,&
  ...
  IF ( Info(1)/=0.AND.Info(1)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DES','IN DEABM, INFO(1) MUST BE '//&
  ...
  IF ( Info(2)/=0.AND.Info(2)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DES','IN DEABM, INFO(2) MUST BE 0 OR 1 '//&
  ...
  IF ( Info(3)/=0.AND.Info(3)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DES','IN DEABM, INFO(3) MUST BE 0 OR 1 '//&
  ...
  IF ( Info(4)/=0.AND.Info(4)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DES','IN DEABM, INFO(4) MUST BE 0 OR 1 '//&
  ...
  IF ( Neq<1 ) THEN
    WRITE (xern1,'(I8)') Neq
    CALL XERMSG('SLATEC','DES','IN DEABM, THE NUMBER OF EQUATIONS '//&
  ...
  DO k = 1, Neq
    IF ( nrtolp==0.AND.Rtol(k)<0. ) THEN
      ...
      CALL XERMSG('SLATEC','DES','IN DEABM, THE RELATIVE ERROR '//&
        'TOLERANCES RTOL MUST BE NON-NEGATIVE.  YOU HAVE '//&
        'CALLED THE CODE WITH  RTOL('//xern1//') = '//xern3//&
    ...
    IF ( natolp==0.AND.Atol(k)<0. ) THEN
      ...
      CALL XERMSG('SLATEC','DES','IN DEABM, THE ABSOLUTE ERROR '//&
        'TOLERANCES ATOL MUST BE NON-NEGATIVE.  YOU HAVE '//&
        'CALLED THE CODE WITH  ATOL('//xern1//') = '//xern3//&
        '.  IN THE CASE OF VECTOR ERROR TOLERANCES, '//&
  ...
  IF ( Info(4)==1 ) THEN
    IF ( SIGN(1.,Tout-T)/=SIGN(1.,Tstop-T).OR.ABS(Tout-T)>ABS(Tstop-T) )&
      ...
      CALL XERMSG('SLATEC','DES','IN DEABM, YOU HAVE CALLED THE '//&
  ...
  IF ( Init/=0 ) THEN
    ...
    IF ( T/=Told ) THEN
      ...
      CALL XERMSG('SLATEC','DES','IN DEABM, YOU HAVE CHANGED THE '//&
    ...
    IF ( Init/=1 ) THEN
      IF ( Delsgn*(Tout-T)<0. ) THEN
        WRITE (xern3,'(1PE15.6)') Tout
        CALL XERMSG('SLATEC','DES','IN DEABM, BY CALLING THE '//&
  ...
  IF ( Idid==(-33) ) THEN
    IF ( Iquit/=(-33) ) THEN
      ...
      Info(1) = -1
    ELSE
      CALL XERMSG('SLATEC','DES','IN DEABM, INVALID INPUT WAS '//&
