!*==DBINTK.f90  processed by SPAG 6.72Dc at 11:01 on  6 Feb 2019
!DECK DBINTK
SUBROUTINE DBINTK(X,Y,T,N,K,Bcoef,Q,Work)
  ...
  ELSE
    IF ( N<K ) THEN
      CALL XERMSG('SLATEC','DBINTK','N DOES NOT SATISFY N.GE.K',2,1)
      RETURN
    ELSE
      jj = N - 1
      IF ( jj/=0 ) THEN
