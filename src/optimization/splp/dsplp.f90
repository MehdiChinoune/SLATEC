...
SUBROUTINE DSPLP(DUSRMT,Mrelas,Nvars,Costs,Prgopt,Dattrv,Bl,Bu,Ind,Info,&
  ...
  IF ( Mrelas<=0 ) THEN
    ...
    CALL XERMSG('DSPLP','VALUE OF MRELAS MUST BE .GT. 0.  NOW = '//xern1,5,1)
    Info = -5
    RETURN
  END IF
  !
  IF ( Nvars<=0 ) THEN
    ...
    CALL XERMSG('DSPLP','VALUE OF NVARS MUST BE .GT. 0.  NOW = '//xern1,6,1)
    Info = -6
    RETURN
  END IF
  !
  ...
  END DO
  nerr = 21
  CALL XERMSG('DSPLP','USER-DEFINED VALUE OF LBM MUST BE .GE. 0.',&
    nerr,iopt)
  Info = -nerr
  RETURN
  !
