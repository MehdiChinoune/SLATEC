!** DDCST
PURE SUBROUTINE DDCST(Maxord,Mint,Iswflg,El,Tq)
  ...
  REAL(DP), INTENT(OUT) :: El(13,12), Tq(3,12)
  !
  REAL(DP) :: factrl(12), gama(14), summ
  INTEGER :: i, j, mxrd
  ...
  IF( Mint==1 ) THEN
    ...
    END DO
    DO j = 1, Maxord
      Tq(1,j) = -1._DP/(factrl(j)*gama(j))
      Tq(2,j) = -1._DP/gama(j+1)
      Tq(3,j) = -1._DP/gama(j+2)
    END DO
  ...
  ELSEIF( Mint==2 ) THEN
    ...
    END DO
    DO j = 1, Maxord
      IF( j>1 ) Tq(1,j) = 1._DP/factrl(j-1)
      Tq(2,j) = (j+1)/El(1,j)
      Tq(3,j) = (j+2)/El(1,j)
    END DO
  END IF
