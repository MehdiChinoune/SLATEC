...
PURE SUBROUTINE HWSSS1(Ts,Tf,M,Mbdcnd,Bdts,Bdtf,Ps,Pf,N,Nbdcnd,Bdps,Bdpf,&
  ...
  dth2 = dth*dth
  cp = 4._SP/(fn*dth2)
  wp = fn*SIN(hdth)/4._SP
  DO i = 1, mp1
    fim1 = i - 1
    theta = fim1*dth + Ts
    Sint(i) = SIN(theta)
    IF( Sint(i)/=0 ) THEN
      t1 = 1._SP/(dth2*Sint(i))
  ...
  SELECT CASE (mbr)
    ...
    CASE DEFAULT
      IF( Nbdcnd==3 ) THEN
        yhld = F(1,jps) - 4._SP/(fn*dphi*dth2)*(Bdpf(2)-Bdps(2))
        DO j = 1, np1
          F(1,j) = yhld
        END DO
      END IF
  ...
  SELECT CASE (mbr)
    ...
    CASE DEFAULT
      IF( Nbdcnd==3 ) THEN
        yhld = F(M+1,jps) - 4._SP/(fn*dphi*dth2)*(Bdpf(M)-Bdps(M))
        DO j = 1, np1
          F(M+1,j) = yhld
        END DO
      END IF
  ...
  IF( inp>0 ) THEN
    ...
    END DO
    dfn = cp*summ
    dnn = cp*((wps+wpf+fjj)*(Sn(2)-1._SP)) + Elmbda
