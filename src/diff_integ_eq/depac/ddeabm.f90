...
SUBROUTINE DDEABM(DF,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,Iwork,Liw)
  ...
  IF ( Iwork(Liw)>=5 ) THEN
    IF ( T==Rwork(21+Neq) ) THEN
      ...
      CALL XERMSG('SLATEC','DDEABM','AN APPARENT INFINITE LOOP HAS BEEN DETECTED.&
  ...
  IF ( Lrw<130+21*Neq ) THEN
    WRITE (xern1,'(I8)') Lrw
    CALL XERMSG('SLATEC','DDEABM','THE LENGTH OF THE RWORK ARRAY MUST BE AT&
  ...
  IF ( Liw<51 ) THEN
    WRITE (xern1,'(I8)') Liw
    CALL XERMSG('SLATEC','DDEABM','THE LENGTH OF THE IWORK ARRAY MUST BE AT LEAST 51.&
