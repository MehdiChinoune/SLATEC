...
SUBROUTINE DEBDF(F,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,Iwork,Liw,JAC)
  ...
  IF ( Iwork(Liw)>=5 ) THEN
    IF ( T==Rwork(21+Neq) ) THEN
      ...
      CALL XERMSG('SLATEC','DEBDF','AN APPARENT INFINITE LOOP HAS BEEN DETECTED.&
  ...
  IF ( Info(1)/=0.AND.Info(1)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DEBDF','INFO(1) MUST BE SET TO 0 FOR THE  START OF&
  ...
  IF ( Info(2)/=0.AND.Info(2)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DEBDF','INFO(2) MUST BE 0 OR 1 INDICATING SCALAR&
  ...
  IF ( Info(3)/=0.AND.Info(3)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DEBDF','INFO(3) MUST BE 0 OR 1 INDICATING THE INTERVAL&
  ...
  IF ( Info(4)/=0.AND.Info(4)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DEBDF','INFO(4) MUST BE 0 OR 1 INDICATING WHETHER OR&
  ...
  IF ( Info(5)/=0.AND.Info(5)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DEBDF','INFO(5) MUST BE 0 OR 1 INDICATING WHETHER THE&
      & CODE IS TOLD TO FORM THE JACOBIAN MATRIX BY NUMERICAL DIFFERENCING OR&
      & YOU PROVIDE A SUBROUTINE TO EVALUATE IT ANALYTICALLY.  YOU HAVE CALLED&
      & THE CODE WITH INFO(5) = '//xern1,15,1)
    Idid = -33
  END IF
  !
  IF ( Info(6)/=0.AND.Info(6)/=1 ) THEN
    WRITE (xern1,'(I8)') Info(6)
    CALL XERMSG('SLATEC','DEBDF','INFO(6) MUST BE 0 OR 1 INDICATING WHETHER&
      & THE CODE IS TOLD TO TREAT THE JACOBIAN AS A FULL (DENSE) MATRIX OR AS&
      & HAVING A SPECIAL BANDED STRUCTURE.  YOU HAVE CALLED THE CODE WITH INFO(6) = '&
      //xern1,16,1)
    Idid = -33
  END IF
  !
  ...
  IF ( Info(6)/=0 ) THEN
    ...
    IF ( ml<0.OR.ml>=Neq.OR.mu<0.OR.mu>=Neq ) THEN
      WRITE (xern1,'(I8)') ml
      WRITE (xern2,'(I8)') mu
      CALL XERMSG('SLATEC','DEBDF','YOU HAVE SET INFO(6) = 1, TELLING THE CODE&
  ...
  IF ( Lrw<250+(10+ilrw)*Neq ) THEN
    WRITE (xern1,'(I8)') Lrw
    IF ( Info(6)==0 ) THEN
      CALL XERMSG('SLATEC','DEBDF','LENGTH OF ARRAY RWORK MUST BE AT LEAST 250&
  ...
  IF ( Liw<56+Neq ) THEN
    WRITE (xern1,'(I8)') Liw
    CALL XERMSG('SLATEC','DEBDF','LENGTH OF ARRAY IWORK BE AT LEAST  56 + NEQ.  YOU HAVE CALLED THE CODE WITH LIW = '//xern1,2,1)
    Idid = -33
  END IF
  !
