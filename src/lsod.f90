...
SUBROUTINE LSOD(F,Neq,T,Y,Tout,Rtol,Atol,Idid,Ypout,Yh,Yh1,Ewt,Savf,Acor,&
  ...
  IF ( Neq<1 ) THEN
    ...
    CALL XERMSG('SLATEC','LSOD',&
      ...
      'INTEGER.$$YOU HAVE CALLED THE CODE WITH NEQ = '//xern1,6,1)
    Idid = -33
  ENDIF
  !
  nrtolp = 0
  ...
  DO k = 1 , Neq
    ...
    IF ( Atol(k)<0. ) THEN
      ...
      CALL XERMSG('SLATEC','LSOD','IN DEBDF, THE ABSOLUTE ERROR '//&
  ...
  IF ( ITStop==1 ) THEN
    IF ( SIGN(1.,Tout-T)/=SIGN(1.,Tstop-T).OR.ABS(Tout-T)>ABS(Tstop-T) )&
      ...
      CALL XERMSG('SLATEC','LSOD','IN DEBDF, YOU HAVE CALLED THE '//&
  ...
  IF ( Idid==(-33) ) THEN
    IF ( IQUit/=(-33) ) THEN
      ...
      IBEgin = -1
    ELSE
      CALL XERMSG('SLATEC','LSOD','IN DEBDF, INVALID INPUT WAS '//&
