...
SUBROUTINE HPSORT(Hx,N,Strbeg,Strend,Iperm,Kflag,Work,Ier)
  !> Return the permutation vector generated by sorting a substring within
  !  a character array and, optionally, rearrange the elements of the array.
  !  The array may be sorted in forward or reverse lexicographical order.
  ...
  !     .. Scalar Arguments ..
  INTEGER, INTENT(IN) :: Kflag, N, Strbeg, Strend
  INTEGER, INTENT(OUT) :: Ier
  CHARACTER(*), INTENT(INOUT) :: Work
  !     .. Array Arguments ..
  INTEGER, INTENT(OUT) :: Iperm(N)
  CHARACTER(*), INTENT(INOUT) :: Hx(N)
  !     .. Local Scalars ..
  REAL(SP) :: r
  ...
  IF( nn<1 ) THEN
    Ier = 1
    ERROR STOP 'HPSORT : The number of values to be sorted, N, is not positive.'
  END IF
  kk = ABS(Kflag)
  IF( kk/=1 .AND. kk/=2 ) THEN
    Ier = 2
    ERROR STOP 'HPSORT : The sort control parameter, KFLAG, is not 2, 1, -1, or -2.'
  END IF
  !
  IF( LEN(Work)<LEN(Hx(1)) ) THEN
    Ier = 3
    ERROR STOP ' HPSORT : The length of the work variable, WORK, is too short.'
  END IF
  IF( Strbeg>Strend ) THEN
    Ier = 4
    ERROR STOP 'HPSORT : The string beginning, STRBEG, is beyond its end, STREND.'
  END IF
  IF( Strbeg<1 .OR. Strbeg>LEN(Hx(1)) ) THEN
    Ier = 5
    ERROR STOP 'HPSORT : The string beginning, STRBEG, is out-of-range.'
  END IF
  IF( Strend<1 .OR. Strend>LEN(Hx(1)) ) THEN
    Ier = 6
    ERROR STOP 'HPSORT : The string end, STREND, is out-of-range.'
  END IF
  !
