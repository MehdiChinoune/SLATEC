!** IPSORT
PURE SUBROUTINE IPSORT(Ix,N,Iperm,Kflag,Ier)
  ...
  IF( nn<1 ) THEN
    Ier = 1
    ERROR STOP 'IPSORT : The number of values to be sorted, N, is not positive.'
  END IF
  kk = ABS(Kflag)
  IF( kk/=1 .AND. kk/=2 ) THEN
    Ier = 2
    ERROR STOP 'IPSORT : The sort control parameter, KFLAG, is not 2, 1, -1, or -2.'
  END IF
  !
