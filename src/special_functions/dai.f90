...
REAL(DP) FUNCTION DAI(X)
  ...
  REAL(DP), PARAMETER :: x3sml = D1MACH(3)**0.3334D0, &
    xmaxt = (-1.5D0*LOG(D1MACH(1)))**0.6667D0, &
    xmax = xmaxt - xmaxt*LOG(xmaxt)/(4.0D0*SQRT(xmaxt)+1.0D0) - 0.01D0
  REAL(DP), PARAMETER :: aifcs(13) = [ -.37971358496669997496197089469414D-1, &
  ...
  REAL(DP), PARAMETER :: aigcs(13) = [ +.18152365581161273011556209957864D-1, &
