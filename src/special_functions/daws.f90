...
REAL(SP) FUNCTION DAWS(X)
  ...
  REAL(SP), PARAMETER :: eps = R1MACH(3), xsml = SQRT(1.5*eps), xbig = SQRT(0.5/eps), &
    xmax = EXP(MIN(-LOG(2.*R1MACH(1)),LOG(R1MACH(2)))-1.0)
  REAL(SP), PARAMETER :: dawcs(13) = [ -.006351734375145949E0,-.22940714796773869E0, &
  ...
  REAL(SP), PARAMETER :: daw2cs(29) = [ -.056886544105215527E0, -.31811346996168131E0, &
  ...
  REAL(SP), PARAMETER :: dawacs(26) = [ .01690485637765704E0, .00868325227840695E0, &
