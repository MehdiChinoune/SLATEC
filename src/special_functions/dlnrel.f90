...
REAL(DP) FUNCTION DLNREL(X)
  ...
  IF( first ) THEN
    nlnrel = INITDS(alnrcs,43,0.1_SP*D1MACH(3))
    first = .FALSE.
  END IF
  !
  IF( X<=(-1._DP) ) CALL XERMSG('DLNREL','X IS LE -1',2,2)
  IF( X<xmin ) CALL XERMSG('DLNREL','ANSWER LT HALF PRECISION BECAUSE X TOO NEAR -1',1,1)
  !
  IF( ABS(X)<=0.375_DP ) THEN
    DLNREL = X*(1._DP-X*DCSEVL(X/.375_DP,alnrcs,nlnrel))
  ELSE
    DLNREL = LOG(1._DP+X)
  END IF
  !
