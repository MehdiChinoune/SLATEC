...
PURE SUBROUTINE DRC3JM(L1,L2,L3,M1,M2min,M2max,Thrcof,Ndim,Ier)
  ...
  IF( (L1-ABS(M1)+eps<0._DP) .OR. (MOD(L1+ABS(M1)+eps,1._DP)>=eps+eps) ) THEN
    Ier = 1
  ...
  ELSEIF( MOD(L1+L2+L3+eps,1._DP)>=eps+eps ) THEN
    Ier = 3
  ...
  IF( MOD(M2max-M2min+eps,1._DP)>=eps+eps ) THEN
    Ier = 4
  ...
  IF( M2min<M2max-eps ) THEN
    ...
    !     MSCALE = 0
    nfin = INT(M2max-M2min+1._DP+eps)
    IF( Ndim<nfin ) THEN
      !
  ...
  ELSEIF( M2min<M2max+eps ) THEN
    ...
    !     MSCALE = 0
    Thrcof(1) = (-1._DP)**INT(ABS(L2-L3-M1)+eps)/SQRT(L1+L2+L3+1._DP)
    RETURN
  ELSE
    !
  ...
  DO
    ...
    !
    oldfac = newfac
    a1 = (L2-m2+1._DP)*(L2+m2)*(L3+m3+1._DP)*(L3-m3)
    newfac = SQRT(a1)
    !
    !
    dv = (L1+L2+L3+1._DP)*(L2+L3-L1) - (L2-m2+1._DP)*(L3+m3+1._DP) - (L2+m2-1._DP)&
      *(L3-m3-1._DP)
    !
    !
    ...
    IF( lstep>2 ) THEN
      ...
      DO
        ...
        m3 = -M1 - m2
        oldfac = newfac
        a1s = (L2-m2+2._DP)*(L2+m2-1._DP)*(L3+m3+2._DP)*(L3-m3-1._DP)
        newfac = SQRT(a1s)
        dv = (L1+L2+L3+1._DP)*(L2+L3-L1) - (L2-m2+1._DP)*(L3+m3+1._DP)&
          - (L2+m2-1._DP)*(L3-m3-1._DP)
        c1 = -dv/newfac
        IF( lstep>2 ) THEN
          !
          c2 = -oldfac/newfac
