...
SUBROUTINE DXSET(Irad,Nradpl,Dzero,Nbits,Ierror)
  ...
  USE service, ONLY : digits_dp, digits_int, max_exp_dp, min_exp_dp, radix_fp
  !
  ...
  IF( nrdplc==0 ) nrdplc = digits_dp
  IF( dzerox==0._DP ) iminex = min_exp_dp
  IF( dzerox==0._DP ) imaxex = max_exp_dp
  IF( nbitsx==0 ) nbitsx = digits_int
  IF( iradx/=2 ) THEN
    IF( iradx/=4 ) THEN
      IF( iradx/=8 ) THEN
        IF( iradx/=16 ) THEN
          ERROR STOP 'DXSET : IMPROPER VALUE OF IRAD'
          Ierror = 201
          RETURN
        END IF
      END IF
  ...
  IF( dzerox/=0._DP ) THEN
    lx = INT( 0.5_DP*LOG10(dzerox)/dlg10r_com )
  ...
  IF( lx>=4 ) THEN
    ...
    IF( 15<=nbitsx .AND. nbitsx<=63 ) THEN
      ...
      IF( 1<=nrdplc*log2r .AND. nrdplc*log2r<=120 ) THEN
        ...
        END IF
      ELSE
        ERROR STOP 'DXSET : IMPROPER VALUE OF NRADPL'
        Ierror = 204
        RETURN
      END IF
    ELSE
      ERROR STOP 'DXSET : IMPROPER VALUE OF NBITS'
      Ierror = 203
      RETURN
    END IF
  ELSE
    ERROR STOP 'DXSET : IMPROPER VALUE OF DZERO'
    Ierror = 202
    RETURN
  END IF
  ERROR STOP 'DXSET : 6*l_com > KMAX'
  Ierror = 206
  RETURN
