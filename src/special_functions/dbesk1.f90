...
REAL(DP) FUNCTION DBESK1(X)
  ...
  REAL(DP) :: y
  INTEGER, SAVE :: ntk1
  REAL(DP), PARAMETER :: xmin = EXP(MAX(LOG(D1MACH(1)),-LOG(D1MACH(2)))+0.01D0), &
    xsml = SQRT(4.0D0*D1MACH(3)), xmaxt = -LOG(D1MACH(1)), &
    xmax = xmaxt - 0.5D0*xmaxt*LOG(xmaxt)/(xmaxt+0.5D0)
  REAL(DP), PARAMETER :: bk1cs(16) = [ +.25300227338947770532531120868533D-1, &
  ...
  IF ( first ) THEN
    ntk1 = INITDS(bk1cs,16,0.1*D1MACH(3))
    first = .FALSE.
  END IF
  !
  IF ( X<=0.D0 ) CALL XERMSG('DBESK1','X IS ZERO OR NEGATIVE',2,2)
  IF ( X>2.0D0 ) THEN
    !
    DBESK1 = 0.D0
