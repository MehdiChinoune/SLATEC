...
REAL(SP) FUNCTION ALNREL(X)
  ...
  REAL(SP) :: X
  INTEGER, SAVE :: nlnrel
  REAL(SP), PARAMETER :: xmin = -1._SP + SQRT(R1MACH(4))
  ...
  IF( first ) THEN
    nlnrel = INITS(alnrcs,23,0.1_SP*R1MACH(3))
  END IF
  !
  IF( X<=(-1._SP) ) CALL XERMSG('ALNREL','X IS LE -1',2,2)
  IF( X<xmin ) CALL XERMSG('ALNREL',&
    'ANSWER LT HALF PRECISION BECAUSE X TOO NEAR -1',1,1)
  !
  IF( ABS(X)<=0.375_SP ) THEN
    ALNREL = X*(1._SP-X*CSEVL(X/.375_SP,alnrcs,nlnrel))
  ELSE
    ALNREL = LOG(1._SP+X)
  END IF
  !
