...
SUBROUTINE DDEBDF(DF,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,Iwork,Liw,&
  ...
  IF ( Iwork(Liw)>=5 ) THEN
    IF ( T==Rwork(21+Neq) ) THEN
      ...
      CALL XERMSG('SLATEC','DDEBDF','AN APPARENT INFINITE LOOP HAS BEEN DETECTED.&
  ...
  IF ( Info(1)/=0.AND.Info(1)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DDEBDF','INFO(1) MUST BE SET TO 0 FOR THE  START&
  ...
  IF ( Info(3)/=0.AND.Info(3)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DDEBDF','INFO(3) MUST BE 0 OR 1 INDICATING THE&
  ...
  IF ( Info(4)/=0.AND.Info(4)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DDEBDF','INFO(4) MUST BE 0 OR 1 INDICATING WHETHER&
  ...
  IF ( Info(5)/=0.AND.Info(5)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DDEBDF','INFO(5) MUST BE 0 OR 1 INDICATING WHETHER&
  ...
  IF ( Info(6)/=0.AND.Info(6)/=1 ) THEN
    ...
    CALL XERMSG('SLATEC','DDEBDF','INFO(6) MUST BE 0 OR 1 INDICATING WHETHER&
  ...
  IF ( Info(6)/=0 ) THEN
    ...
    IF ( ml<0.OR.ml>=Neq.OR.mu<0.OR.mu>=Neq ) THEN
      WRITE (xern1,'(I8)') ml
      WRITE (xern2,'(I8)') mu
      CALL XERMSG('SLATEC','DDEBDF','YOU HAVE SET INFO(6) = 1,&
