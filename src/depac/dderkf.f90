...
SUBROUTINE DDERKF(DF,Neq,T,Y,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,Iwork,Liw,&
  ...
  IF ( Iwork(Liw)>=5 ) THEN
    IF ( T==Rwork(21+Neq) ) THEN
      ...
      CALL XERMSG('SLATEC','DDERKF','AN APPARENT INFINITE LOOP HAS BEEN DETECTED.&
  ...
  IF ( Lrw<30+7*Neq ) THEN
    WRITE (xern1,'(I8)') Lrw
    CALL XERMSG('SLATEC','DDERKF','LENGTH OF RWORK ARRAY MUST BE AT LEAST&
  ...
  IF ( Liw<34 ) THEN
    WRITE (xern1,'(I8)') Liw
    CALL XERMSG('SLATEC','DDERKF','LENGTH OF IWORK ARRAY MUST BE AT LEAST 34.  YOU HAVE CALLED THE CODE WITH LIW = '//xern1,2,1)
    Idid = -33
  ENDIF
  !
