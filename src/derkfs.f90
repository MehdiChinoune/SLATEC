...
!
      IF ( Info(1)/=0.AND.Info(1)/=1 ) THEN
        ...
        CALL XERMSG('SLATEC','DERKFS','IN DERKF, INFO(1) MUST BE SET TO 0 '//
...
!
      ...
      DO k = 1 , Neq
        IF ( nrtolp==0.AND.Rtol(k)<0.D0 ) THEN
          ...
          CALL XERMSG('SLATEC','DERKFS','IN DERKF, THE RELATIVE ERROR '//
...
!
        IF ( natolp==0.AND.Atol(k)<0.D0 ) THEN
          ...
          CALL XERMSG('SLATEC','DERKFS','IN DERKF, THE ABSOLUTE ERROR '//
...
!
      IF ( Init/=0 ) THEN
        IF ( T==Tout ) THEN
          ...
          CALL XERMSG('SLATEC','DERKFS','IN DERKF, YOU HAVE CALLED THE '//
...
!
        IF ( T/=Told ) THEN
          ...
          CALL XERMSG('SLATEC','DERKFS','IN DERKF, YOU HAVE CHANGED THE '//
...
!
        IF ( Init/=1 ) THEN
          IF ( Dtsign*(Tout-T)<0.D0 ) THEN
            ...
            CALL XERMSG('SLATEC','DERKFS','IN DERKF, BY CALLING THE CODE '//
...
!
      IF ( Idid==(-33) ) THEN
        IF ( Iquit/=(-33) ) THEN
          ...
          GOTO 300
        ELSE
          CALL XERMSG('SLATEC','DERKFS','IN DERKF, INVALID INPUT WAS '//
