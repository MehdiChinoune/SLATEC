...
SUBROUTINE QAWOE(F,A,B,Omega,Integr,Epsabs,Epsrel,Limit,Icall,Maxp1,&
  ...
  REAL A, abseps, Abserr, Alist(*), area, area1, area12, area2, a1, &
    a2, B, Blist(*), b1, b2, Chebmo(Maxp1,25), correc, defab1, defab2, &
    defabs, domega, dres, Elist(*), epmach, Epsabs, Epsrel, &
    erlarg, erlast, errbnd, errmax, error1, erro12, error2, &
    errsum, ertest, oflow, Omega, resabs, reseps, Result, &
  ...
  IF ( Ier/=6 ) THEN
    ...
    ELSE
      ...
      DO Last = 2, Limit
        ...
        b2 = Blist(maxerr)
        erlast = errmax
        CALL QC25F(F,a1,b1,domega,Integr,nrmom,Maxp1,0,area1,error1,nev,&
          resabs,defab1,Momcom,Chebmo)
        Neval = Neval + nev
        CALL QC25F(F,a2,b2,domega,Integr,nrmom,Maxp1,1,area2,error2,nev,&
