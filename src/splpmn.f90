...
!     PROCEDURE (PRINT PROLOGUE)
        ...
        idum(1) = Nvars + Mrelas
        CALL IVOUT(1,idum,
     &'('' DIMENSIONS OF BL(*),BU(*),IND(*)''        /'' PRIMAL(*),DUALS(*) ='')
     &',idg)
        CALL IVOUT(1,idum,'('' DIMENSION OF IBASIS(*)='')',idg)
        idum(1) = lprg + 1
        CALL IVOUT(1,idum,'('' DIMENSION OF PRGOPT(*)='')',idg)
        CALL IVOUT(0,idum,
     &'('' 1-NVARS=INDEPENDENT VARIABLE INDICES.''/                     '' (NVAR
     &S+1)-(NVARS+MRELAS)=DEPENDENT VARIABLE INDICES.''/        '' CONSTRAINT IN
     &DICATORS ARE 1-4 AND MEAN'')',idg)
        CALL IVOUT(0,idum,
     &'('' 1=VARIABLE HAS ONLY LOWER BOUND.''/                          '' 2=VAR
     &IABLE HAS ONLY UPPER BOUND.''/                            '' 3=VARIABLE HA
     &S BOTH BOUNDS.''/                                 '' 4=VARIABLE HAS NO BOU
     &NDS, IT IS FREE.'')',idg)
     ...
     &             '('' UPPER BOUNDS FOR VARIABLES  (IGNORE UNUSED ENTRIES.)'')'
     &             ,idg)
        IF ( kprint>=2 ) THEN
          CALL IVOUT(0,idum,
     &'(''0NON-BASIC INDICES THAT ARE NEGATIVE SHOW VARIABLES''         '' EXCHA
     &NGED AT A ZERO''/'' STEP LENGTH'')',idg)
          CALL IVOUT(0,idum,
     &'('' WHEN COL. NO. LEAVING=COL. NO. ENTERING, THE ENTERING ''     ''VARIAB
     &LE MOVED''/'' TO ITS BOUND.  IT REMAINS NON-BASIC.''/     '' WHEN COL. NO.
     & OF BASIS EXCHANGED IS NEGATIVE, THE LEAVING''/   '' VARIABLE IS AT ITS UP
     &PER BOUND.'')',idg)
        ENDIF
      ENDIF
...
!++  CODE FOR OUTPUT=YES IS ACTIVE
 2700 IF ( kprint>=2 ) CALL IVOUT(Mrelas,Ibasis,
...
!     COPY RHS INTO WW(*), THEN UPDATE WW(*).
 3100 CALL SCOPY(Mrelas,Rhs,1,Ww,1)
      j = 1
      n20206 = Mrelas
      DO WHILE ( (n20206-j)>=0 )
        ibas = Ibasis(j)
...
!     COPY RHS INTO WW(*), SOLVE SYSTEM.
 4000 CALL SCOPY(Mrelas,Rhs,1,Ww,1)
      trans = .FALSE.
...
!     PROCEDURE (PRINT SUMMARY)
 4500 idum(1) = Info
      ...
      ENDIF
      rdum(1) = SDOT(Nvars,Costs,1,Primal,1)
