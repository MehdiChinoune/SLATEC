!** DCGN
PURE SUBROUTINE DCGN(N,B,X,Nelt,Ia,Ja,A,Isym,MATVEC,MTTVEC,MSOLVE,Itol,Tol,&
  ...
  REAL(DP), INTENT(IN) :: A(N), B(N), Rwork(*)
  REAL(DP), INTENT(INOUT) :: X(N)
  REAL(DP), INTENT(OUT) :: Atdz(N), Atp(N), Atz(N), Dz(N), P(N), R(N), Z(N)
  !     .. Local Scalars ..
  ...
  END IF
  !
  IF( ISDCGN(N,Itol,Tol,R,Atz,Dz,bnrm,solnrm)==0 ) THEN
    !
    !         ***** iteration loop *****
    !
    DO k = 1, Itmax
      ...
      IF( ISDCGN(N,Itol,Tol,R,Atz,Dz,bnrm,solnrm)/=0 ) RETURN
      !
    END DO
    !
    !         *****   end of loop  *****
