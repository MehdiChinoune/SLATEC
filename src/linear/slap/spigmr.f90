!** SPIGMR
PURE SUBROUTINE SPIGMR(N,R0,Sr,Sz,Jscal,Maxl,Maxlp1,Kmp,Nrsts,Jpre,MATVEC,&
  ...
  !         call to SGMRES.  If NRSTS > 0, then the residual
  ...
  !         (KMP<MAXL), and/or when using restarting.
  ...
  REAL(SP), INTENT(IN) :: Bnrm, Tol
  REAL(SP), INTENT(OUT) :: Rhol
  !     .. Array Arguments ..
  INTEGER, INTENT(IN) :: Ia(Nelt), Ipar(*), Ja(Nelt)
  REAL(SP), INTENT(IN) :: A(Nelt), Rpar(*), Sr(N), Sz(N), X(N)
  REAL(SP), INTENT(INOUT) :: Dl(N), R0(N), Wk(N), Xl(N)
  REAL(SP), INTENT(OUT) :: Hes(Maxlp1,Maxl), Q(2*Maxl), V(N,Maxlp1), Z(N)
  !     .. Local Scalars ..
  REAL(SP) :: c, dlnrm, prod, r0nrm, rho, s, snormw, tem, solnrm
  ...
  !
  IF( ISSGMR(N,X,Xl,MSOLVE,Itol,Tol,iter,V(1,1),Wk,Rpar,Ipar,r0nrm, &
  ...
  DO ll = 1, Maxl
    ...
    IF( ISSGMR(N,X,Xl,MSOLVE,Itol,Tol,iter,Dl,Wk,Rpar,Ipar,Rhol, &
