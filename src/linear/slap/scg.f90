!** SCG
PURE SUBROUTINE SCG(N,B,X,Nelt,Ia,Ja,A,Isym,MATVEC,MSOLVE,Itol,Tol,Itmax,Iter,&
    Ierr,R,Z,P,Dz,Rwork,Iwork)
  !> Preconditioned Conjugate Gradient Sparse Ax=b Solver.
  !  Routine to solve a symmetric positive definite linear system
  !  Ax = b  using the Preconditioned Conjugate Gradient method.
  !***
  ! **Library:**   SLATEC (SLAP)
  !***
  ! **Category:**  D2B4
  ...
  !***
  ! **Keywords:**  ITERATIVE PRECONDITION, SLAP, SPARSE, SYMMETRIC LINEAR SYSTEM
  !***
  ...
  REAL(SP), INTENT(INOUT) :: Dz(N), X(N)
  REAL(SP), INTENT(OUT) :: P(N), R(N), Z(N)
  !     .. Local Scalars ..
  ...
  END IF
  !
  IF( ISSCG(N,Itol,Tol,R,Z,Dz,bnrm,solnrm)==0 ) THEN
    !
    !         ***** Iteration loop *****
    !
    DO k = 1, Itmax
      ...
      IF( ISSCG(N,Itol,Tol,R,Z,Dz,bnrm,solnrm)/=0 ) RETURN
      !
    END DO
    !
    !         *****   end of loop  *****
