!*==SPLPUP.f90  processed by SPAG 6.72Dc at 11:02 on  6 Feb 2019
!DECK SPLPUP
SUBROUTINE SPLPUP(USRMAT,Mrelas,Nvars,Prgopt,Dattrv,Bl,Bu,Ind,Info,Amat,&
  ...
  DO j = 1 , Nvars
    ...
    IF ( Ind(j)==3 ) THEN
      IF ( Bl(j)>Bu(j) ) THEN
        ...
        CALL XERMSG('SLATEC','SPLPUP','IN SPLP, LOWER BOUND = '//xern3//&
  ...
  DO
    ...
    IF ( i<1.OR.i>Mrelas.OR.j<1.OR.j>Nvars ) THEN
      ...
      WRITE (xern1,'(I8)') i
      WRITE (xern2,'(I8)') j
      CALL XERMSG('SLATEC','SPLPUP','IN SPLP, ROW INDEX = '//xern1//&
    ...
    ELSE
      ...
      CALL XERMSG('SLATEC','SPLPUP','IN SPLP, INDICATION FLAG = '//xern1//&
