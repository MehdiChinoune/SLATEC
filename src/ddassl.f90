...
SUBROUTINE DDASSL(RES,Neq,T,Y,Yprime,Tout,Info,Rtol,Atol,Idid,Rwork,Lrw,&
    ...
    ELSE
      ...
      IF ( Info(9)/=0 ) THEN
        ...
        IF ( mxord<1.OR.mxord>5 ) THEN
          ...
          CALL XERMSG('SLATEC','DDASSL','MAXORD = '//xern1//' NOT IN RANGE',&
            3,1)
          GOTO 1200
        ENDIF
      ENDIF
      ...
      ELSE
        ...
        IF ( Info(7)/=0 ) THEN
          ...
          IF ( hmax<=0.0D0 ) THEN
            ...
            CALL XERMSG('SLATEC','DDASSL','HMAX = '//xern3//' .LT. 0.0',10,&
              1)
            GOTO 1200
          ENDIF
        ENDIF
  ...
  IF ( nzflg==0 ) THEN
    !
    CALL XERMSG('SLATEC','DDASSL','ALL ELEMENTS OF RTOL AND ATOL ARE ZERO',&
      8,1)
    GOTO 1200
  ELSE
    !
      ...
      IF ( Info(4)==1 ) THEN
        IF ( Info(3)==1 ) THEN
          ...
          IF ( (tn-T)*h>0.0D0 ) THEN
            ...
            ELSE
              CALL DDATRP(tn,tn,Y,Yprime,Neq,Iwork(LKOLD),Rwork(lphi),&
                Rwork(LPSI))
              T = tn
              Idid = 1
              done = .TRUE.
