!** DLSEI
PURE SUBROUTINE DLSEI(W,Mdw,Me,Ma,Mg,N,Prgopt,X,Rnorme,Rnorml,Mode,Ws,Ip)
  ...
  REAL(DP), INTENT(INOUT) :: W(Mdw,N+1), Ws(*)
  REAL(DP), INTENT(OUT) :: Rnorme, Rnorml, X(N)
  !
  ...
  IF( Ip(1)>0 ) THEN
    ...
    IF( Ip(1)<lchk ) THEN
      WRITE (xern1,'(I8)') lchk
      ERROR STOP 'DLSEI : INSUFFICIENT STORAGE ALLOCATED FOR WS(*)'
      RETURN
    END IF
  END IF
  !
  IF( Ip(2)>0 ) THEN
    ...
    IF( Ip(2)<lchk ) THEN
      WRITE (xern1,'(I8)') lchk
      ERROR STOP 'DLSEI : INSUFFICIENT STORAGE ALLOCATED FOR IP(*)'
      RETURN
    END IF
  END IF
  !
  ...
  !
  IF( Mdw<m ) THEN
    ERROR STOP 'DLSEI : MDW<ME+MA+MG IS AN ERROR'
    RETURN
  END IF
  !
  np1 = N + 1
  ...
  IF( link==0 .OR. link>nlink ) THEN
    ERROR STOP 'DLSEI : THE OPTION VECTOR IS UNDEFINED'
    RETURN
  END IF
  DO
    !
    IF( link>1 ) THEN
      ntimes = ntimes + 1
      IF( ntimes>nopt ) THEN
        ERROR STOP 'DLSEI : THE LINKS IN THE OPTION VECTOR ARE CYCLING.'
        RETURN
      END IF
      !
    ...
    IF( cov .AND. Mdw<N ) THEN
      ERROR STOP 'DLSEI : MDW < N WHEN COV MATRIX NEEDED, IS AN ERROR'
      RETURN
    END IF
    !
