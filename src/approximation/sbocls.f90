...
SUBROUTINE SBOCLS(W,Mdw,Mcon,Mrows,Ncols,Bl,Bu,Ind,Iopt,X,Rnormc,Rnorm,Mode,Rw,Iw)
  ...
  REAL(SP) :: anorm, cnorm, srelpr, t, t1, t2, wt
  ...
  IF( Mcon>0 .AND. filter ) THEN
    ...
    irw = Ncols + 1
    iiw = 1
    CALL SBOLS(W,Mdw,Mcon,Ncols+Mcon,Bl,Bu,Ind,jopt,X,Rnormc,modec,Rw(irw),Iw(iiw))
    !
  ...
  SELECT CASE (iscale)
    ...
    CASE (3)
      ...
      X(Ncols+Mcon+1:2*Ncols+Mcon) = 1._SP
    CASE (4)
      !     CASE 4
      !
  ...
  X(2*(Ncols+Mcon)+2) = 1._SP/wt
  m = mout + Mcon
  CALL SBOLS(W,Mdw,m,Ncols+Mcon,Bl,Bu,Ind,Iopt(lbou),X,Rnorm,Mode,Rw(lrw),Iw(liw))
  Rnorm = Rnorm/wt
  !     END PROCEDURE
