!** PVALUE
PURE SUBROUTINE PVALUE(L,Nder,X,Yfit,Yp,A)
  !> Use the coefficients generated by POLFIT to evaluate the polynomial fit of
  !  degree L, along with the first NDER of its derivatives, at a specified point.
  !***
  ! **Library:**   SLATEC
  !***
  ! **Category:**  K6
  ...
  IF( L<0 ) THEN
    !
    ERROR STOP 'PVALUE : INVALID INPUT PARAMETER.  ORDER OF POLYNOMIAL EVALUATION &
      &REQUESTED IS NEGATIVE -- EXECUTION TERMINATED.'
    RETURN
  ELSE
    ndo = MAX(Nder,0)
    ndo = MIN(ndo,L)
  ...
  WRITE (xern1,'(I8)') L
  WRITE (xern2,'(I8)') nord
  ERROR STOP 'PVALUE : THE ORDER OF POLYNOMIAL EVALUATION, L REQUESTED EXCEEDS &
    &THE HIGHEST ORDER FIT, NORD, COMPUTED BY POLFIT -- EXECUTION TERMINATED.'
  !
  RETURN
