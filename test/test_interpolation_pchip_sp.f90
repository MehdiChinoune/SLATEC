...
CONTAINS
  ...
  SUBROUTINE EVCHCK(Lout,Kprint,Npts,Xev,Fev,Dev,Fev2,Fail)
    ...
    !           Also moved formats to end to be consistent with other PCHIP quick checks.
    ...
    USE interpolation, ONLY : CHFDV, CHFEV
  ...
  SUBROUTINE EVERCK(Lout,Kprint,Fail)
    ...
    USE interpolation, ONLY : CHFDV, CHFEV, PCHFD, PCHFE
    !
    !  Declare arguments.
    !
    INTEGER :: Lout, Kprint
  ...
  SUBROUTINE EVPCCK(Lout,Kprint,X,Y,F,Fx,Fy,Xe,Ye,Fe,De,Fe2,Fail)
    ...
    USE service, ONLY : eps_sp
    USE interpolation, ONLY : PCHFD, PCHFE
    !
    !  Declare arguments.
    !
    INTEGER :: Lout, Kprint
...
!    IF( fail ) ifail = ifail + 4
  ...
  SUBROUTINE PCHQK3(Lun,Kprint,Ipass)
    ...
    !   930317  Improved output formats.  (FNF)
    USE service, ONLY : eps_sp
    USE interpolation, ONLY : PCHIC, PCHIM, PCHSP
    !
    !*Internal Notes:
    !
  ...
  SUBROUTINE PCHQK5(Lun,Kprint,Ipass)
    ...
    !   930317  Improved output formats.  (FNF)
    USE service, ONLY : eps_sp
    USE interpolation, ONLY : BVALU, PCHBS
    !
    !*Internal Notes:
