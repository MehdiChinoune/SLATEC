...
CONTAINS
  ...
  SUBROUTINE DEVCHK(Lout,Kprint,Npts,Xev,Fev,Dev,Fev2,Fail)
    ...
    !   910801  Added EXTERNAL statement for RAND due to problem on IBM RS 6000.  (WRB)
    !   910819  Changed argument to RAND function from a D.P. zero to a S.P. zero.  (WRB)
    USE service, ONLY : eps_dp
    USE interpolation, ONLY : DCHFDV, DCHFEV
  ...
  SUBROUTINE DEVERK(Lout,Kprint,Fail)
    ...
    USE interpolation, ONLY : DCHFDV, DCHFEV, DPCHFD, DPCHFE
    !
    !  Declare arguments.
    !
    INTEGER :: Lout, Kprint
  ...
  SUBROUTINE DEVPCK(Lout,Kprint,X,Y,F,Fx,Fy,Xe,Ye,Fe,De,Fe2,Fail)
    ...
    USE service, ONLY : eps_dp
    USE interpolation, ONLY : DPCHFD, DPCHFE
    !
    !  Declare arguments.
    !
    INTEGER :: Lout, Kprint
...
!    IF( fail ) ifail = ifail + 4
  ...
  SUBROUTINE DPCHQ3(Lun,Kprint,Ipass)
    ...
    !   930317  Improved output formats.  (FNF)
    USE service, ONLY : eps_dp
    USE interpolation, ONLY : DPCHIC, DPCHIM, DPCHSP
    !
    !*Internal Notes:
    !
  ...
  SUBROUTINE DPCHQ4(Lun,Kprint,Ipass)
    ...
    !   900314  Changed name from PCHQK3 to PCHQK4 and improved some output formats.
    !   900315  Revised prologue and improved some output formats.  (FNF)
    !   900320  Converted to double precision.
    !   900321  Removed IFAIL from call sequence for SLATEC standards and
    !           made miscellaneous cosmetic changes.  (FNF)
  ...
  SUBROUTINE DPCHQ5(Lun,Kprint,Ipass)
    ...
    !   930317  Improved output formats.  (FNF)
    USE service, ONLY : eps_dp
    USE interpolation, ONLY : DBVALU, DPCHBS
    !
    !*Internal Notes:
