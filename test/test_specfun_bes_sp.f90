...
CONTAINS
  ...
  SUBROUTINE BIKCK(Lun,Kprint,Ipass)
    ...
    !           obtain more information when there is failure of the Wronskian.  (RWC)
    !   910501  Added PURPOSE and TYPE records.  (WRB)
    !   910708  Code revised to test error returns for all values of KPRINT.  (WRB)
    USE service, ONLY : eps_sp, tiny_sp, huge_sp
    USE special_functions, ONLY : BESI, BESK
    !
    INTEGER :: Ipass, Kprint
    INTEGER :: i, ix, k, kode, Lun, m, n, nu, nw, ny
...
!    END IF
  ...
  SUBROUTINE BJYCK(Lun,Kprint,Ipass)
    ...
    USE special_functions, ONLY : BESJ, BESY
    !
    INTEGER :: Ipass, Kprint
    INTEGER :: i, ix, k, Lun, m, n, nu, ny
...
!    END IF
  ...
  SUBROUTINE EG8CK(Lun,Kprint,Ipass)
    ...
    USE special_functions, ONLY : EXINT
