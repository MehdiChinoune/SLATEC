...
CONTAINS
  ...
  SUBROUTINE DCMPAR(Icnt,Itest)
    !> Compare values in COMMON block DCHECK for quick check routine DPFITT.
    !***
    ! **Library:**   SLATEC
    !***
  ...
  SUBROUTINE DPFITT(Lun,Kprint,Ipass)
    ...
    !   901205  Changed usage of eps_2_dp to eps_dp and modified the FORMATs.  (RWC)
...
!    END IF
  ...
  SUBROUTINE DNLS1Q(Lun,Kprint,Ipass)
    ...
    IF( info==infos .AND. ABS(temp1-1._DP)<tol2 .AND. ABS(temp2)<tol2 .AND. &
        ABS(temp3-1._DP)<tol2 ) THEN
      fatal = .FALSE.
...
!    END IF
  ...
  SUBROUTINE DFCQX(Lun,Kprint,Ipass)
    ...
    !           to use eps_dp rather than eps_2_dp and cleaned up FORMATs.  (RWC)
    !   930214  Declarations sections added, code revised to test error
    !           returns for all values of KPRINT and code polished.  (WRB)
    USE service, ONLY : eps_dp, DVOUT, IVOUT
    USE approximation, ONLY : DCV, DFC, DMOUT
