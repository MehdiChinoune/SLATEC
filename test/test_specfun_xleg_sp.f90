...
CONTAINS
  ...
  SUBROUTINE FCNQX1(Lun,Kprint,Ipass)
    ...
    USE service, ONLY : digits_sp, max_exp_sp, radix_fp
    USE special_functions, ONLY : XCON, XLEGF, XNRMP, XSET
    !
    ...
    nradpl = 0
    nbits = 0
    CALL XSET(irad,nradpl,0._SP,nbits,ierror)
    IF( ierror/=0 ) Ipass = 0
    ierr = 0
    dnu1 = 2000.4_SP
...
!    Ipass = 0
  ...
  SUBROUTINE XCSRT(Dnu1,Nudiff,Mu1,Mu2,Theta,P,Q,R,Ip,Iq,Ir,C1,Ic1,C2,Ic2,Ierror)
    ...
    !   901019  Revisions to prologue.  (DWL and WRB)
    !   901106  Changed all specific intrinsics to generic.  (WRB)
    USE special_functions, ONLY : XADD, XADJ, XRED
    !
